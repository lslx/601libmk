...
mirror::Class* ClassLinker::FindClass(Thread* self, const char* descriptor,
  ...
  if (descriptor[0] == '[') {
	...
	//	ALOGD("WTF fk32");
      ...
      mirror::Class* tmp= DefineClass(self, descriptor, hash, NullHandle<mirror::ClassLoader>(), *pair.first,
                         *pair.second);
	//	ALOGD("WTF fk4");
...
*/
bool get_pack_name(char *pack_name) {
	...
	ALOGD("WTF get_pack_name failed");
	return false;
}

bool get_dump_path(char *dump_path, const DexFile& dex_file) {
    // mytestxx
	char pack_name[256]={0};
	if(get_pack_name(pack_name)) {
		...
		if(0 != access(dump_path, F_OK)) {
			...
			if(0!=ret){
				ALOGD("WTF mkdir ret=%d,errno=%d, %s",ret,errno,strerror(errno));
				return false;
			}
		}//else nothing to do 
		char *ptr = strrchr(dex_file.GetLocation().c_str(), '/');
		if(ptr) {
			strcat(dump_path, ptr);
		}
		else {
			strcat(dump_path, "/");
			strcat(dump_path, dex_file.GetLocation().c_str());
		}
		sprintf(dump_path, "%s_0x%08x/", dump_path, dex_file.GetLocationChecksum());
		if(0 != access(dump_path, F_OK)) {
			int ret=mkdir(dump_path, S_IRWXU|S_IRWXG|S_IRWXO);
			if(0!=ret){
				ALOGD("WTF mkdir ret=%d,errno=%d, %s",ret,errno,strerror(errno));
	...
	// final check
	if(0 != access(dump_path, F_OK)){
			ALOGD("WTF get_dump_path failed");
			return false;
...
void dump_class(char* dump_path, const DexFile& dex_file,Handle<mirror::ClassLoader> class_loader,ClassLinker* cl) {
  ...
  Thread *self=Thread::Current();
	...
	sprintf(log_info, "Dump class begin : %lu ms\n", (unsigned long)(uint64_t)time);
	...
	if(fd) {
		write(fd, dex_file.Begin(), dex_file.class_defs_off_);
		close(fd);
	}
	sprintf(log_info, "Dump part_before_dex_class_defs, baseAddr = 0x%x, class_defs_off = 0x%x\n", (int)(size_t)dex_file.Begin(), (int)dex_file.class_defs_off_);
	write_log(dump_path, log_info);
	//dump part_after_dex_class_defs
	strcpy(temp, dump_path);
	strcat(temp, "part_after_dex_class_defs");
	int offset_after_dex_class_defs = dex_file.class_defs_off_+sizeof(DexFile::ClassDef)*dex_file.NumClassDefs();
	int len_after_dex_class_defs = dex_file.Size()-dex_file.class_defs_off_-sizeof(DexFile::ClassDef)*dex_file.NumClassDefs();
	fd = open(temp, O_RDWR|O_CREAT, S_IRWXU|S_IRWXG|S_IRWXO);
	if(fd) {
		write(fd, dex_file.Begin()+offset_after_dex_class_defs, len_after_dex_class_defs);
		close(fd);
	}
	...
	for(uint32_t i=0; i<totalClass; i++) {
		...
		//Handle<mirror::Class> klass=cl->FindClass(self,descriptor,class_loader);
		//mirror::Class* klass=cl->FindClass(self,descriptor,class_loader);
		StackHandleScope<1> hs(self);
		//ALOGD("WTF fk1 tid=%ld",syscall(__NR_gettid));
		 Handle<mirror::Class> klass(hs.NewHandle(cl->FindClass(self,descriptor, class_loader)));
		 //ALOGD("WTF fk2 tid=%ld",syscall(__NR_gettid));
		//log_and_clear_exception(dump_path,self);
		self->ClearException();
		if(klass.Get() == nullptr) {
			sprintf(log_info, "Call getClassObject fail, class = %s\n", descriptor);
			write_log(dump_path, log_info);
			write(fd_dex_class_defs, &class_def, sizeof(DexFile::ClassDef));
			continue;
		}
		sprintf(log_info, "Call getClassObject success, class = %s\n", descriptor);
		write_log(dump_path, log_info);
		if(klass.Get() != nullptr){
          if(cl->EnsureInitialized(self,klass, true, true)){
			  		sprintf(log_info, "EnsureInitialized success, class = %s\n", descriptor);
					write_log(dump_path, log_info);
        ...
        pClassData = dexReadClassData(&data);
		if(!pClassData) {
		...
		if(pClassData->directMethods) {
			for(uint32_t ia=0; ia<pClassData->header.directMethodsSize; ia++) {
			  ArtMethod *method = klass->GetDirectMethod(ia,sizeof(void*));
              ...
              //const char * name = dex_file.GetMethodName(dex_file.GetMethodId(dex_method_idx));
				...
				//sprintf(log_info, "directMethods[%d], method->accessFlags = 0x%x, method->insns = 0x%x\n", i, pClassData->directMethods[i].accessFlags, (int)pClassData->directMethods[i].insns);
				//write_log(dump_path, log_info);
				
				//uint32_t accessFlags = method->accessFlags & ACC_METHOD_MASK;
				pClassData->directMethods[ia].accessFlags = ac;
				
				if(!method->GetCodeItemOffset() || !method->GetCodeSize() || method->IsNative()) { //ACC_NATIVE
					pClassData->directMethods[ia].codeOff = 0;
					continue;
				}
				//--
				  pClassData->directMethods[ia].codeOff = offset_dex_end;
                  const DexFile::CodeItem * code = dex_file.GetCodeItem(codeitem_off);
                  uint8_t *item=(uint8_t *) code;
                  int code_item_len = 0;

                  if (code->tries_size_) {
                      const uint8_t *handler_data = (const uint8_t *)(DexFile::GetTryItems(*code, code->tries_size_));
				...
				sprintf(log_info, "code_begin = 0x%x, code_len = 0x%x\n", (int)(size_t)item, code_item_len);
				write_log(dump_path, log_info);
				
				write(fd_extra, item, code_item_len);
				offset_dex_end += code_item_len;
				while(offset_dex_end&3) {
					write(fd_extra, &padding, 1);
					offset_dex_end++;
				}
		...
		if(pClassData->virtualMethods) {
			for(uint32_t ib=0; ib<pClassData->header.virtualMethodsSize; ib++) {
			  ArtMethod *method = klass->GetVirtualMethod(ib,sizeof(void*));
              uint32_t ac = (method->GetAccessFlags()) & mask;
              uint32_t codeitem_off = method->GetCodeItemOffset();
              //uint32_t dex_method_idx = method->GetDexMethodIndex();
              //const char * name = dex_file.GetMethodName(dex_file.GetMethodId(dex_method_idx));
				...
				pClassData->virtualMethods[ib].accessFlags = ac;
				
				if(!method->GetCodeItemOffset() || !method->GetCodeSize() || method->IsNative()) { //ACC_NATIVE
					pClassData->virtualMethods[ib].codeOff = 0;
					continue;
				}
				
				//--
				  pClassData->virtualMethods[ib].codeOff = offset_dex_end;
		...
		DexFile::ClassDef *tmp=(DexFile::ClassDef*) malloc(sizeof(DexFile::ClassDef));

       if (!tmp) {
           continue;
       }

       tmp->class_idx_ = class_def.class_idx_;
       tmp->pad1_=class_def.pad1_;
       tmp->pad2_=class_def.pad2_;
       tmp->access_flags_=class_def.access_flags_;
       tmp->annotations_off_= class_def.annotations_off_;
       tmp->class_data_off_=offset_dex_end;//class_def.class_data_off_;
       tmp->interfaces_off_=class_def.interfaces_off_;
       tmp->source_file_idx_=class_def.source_file_idx_;
       tmp->static_values_off_=class_def.static_values_off_;
       tmp->superclass_idx_=class_def.superclass_idx_;

       //if (pass) {
       //    tmp->class_data_off_=0;
       //    tmp->annotations_off_=0;
       //}
      ...
      // uint8_t *p = (uint8_t *)tmp;
		...
		uint8_t* encoded_class_data = dexEncodeClassData(pClassData, encoded_class_data_len);
		...
		write(fd_dex_class_defs, tmp, sizeof(DexFile::ClassDef));
	...
	time=MilliTime();
	sprintf(log_info, "Dump  class end : %lu ms\n", (unsigned long)(uint64_t)time);
	write_log(dump_path, log_info);
...
void dumpDex(const DexFile& dex_file,Handle<mirror::ClassLoader> class_loader,ClassLinker* cl) {
	...
	if(!get_dump_path(dump_path, dex_file)) {
		return;
	}
	
	sprintf(log_info, "tid=%d,pid=%d,uid=%d,gid=%d dump_path = %s\n",(int)syscall(__NR_gettid),getpid(), getuid(),getgid(), dump_path);
	write_log(dump_path, log_info);
...
//static bool bCreateThread=true;
static pthread_mutex_t mutex;

void* dumpClass(void *parament){
  const DexFile &dex_file=(*((struct arg*)parament)->dex_file);
  Handle<mirror::ClassLoader> *class_loader=((struct arg*)parament)->class_loader;
  ClassLinker *cl = ((struct arg*)parament)->cl;
  
	dumpDex(dex_file, *class_loader,cl);
	return 0;
}
...
{
   void*  pointers[3];
   struct link_mapxx* ptr;
};
 static void* libbacktrace = 0;
struct something *ph =0;
...
int btdump()
{
    ...
    if(!libbacktrace)
    {
		libbacktrace = dlopen("/system/lib/libbacktrace.so",RTLD_LOCAL);
		ph = (something*) dlopen(0, RTLD_LOCAL);
    }
    ...
    *(void**) &create = dlsym(libbacktrace,"_ZN9Backtrace6CreateEiiP12BacktraceMap");
    ...
    ALOGD("WTF lm=%p base %p Backtrace count=%d",(void *)ph,(void *)ph->ptr,count);
    for(int i = 0; i < count; i++)
    {
        std::string line = t->FormatFrameData(i);
        ALOGD("WTF %s",line.c_str());
    }
    return 0;
}
	...
	/*
        ...
        if (bCreateThread) {
			   ...
			   //android::CallStack stack;  
				...
				//int fd = open("/data/local/tmpx/stack.txt", O_RDONLY, S_IRWXU|S_IRWXG|S_IRWXO);
